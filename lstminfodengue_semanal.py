...
# Carrega os dados
df = pd.read_csv("Fortaleza-dengue (2).csv")
...
# ============================
# PREVISÃO FUTURA (Ex: 106 semanas à frente)
# ============================

n = 106 # número de semanas futuras
...
#Previsoes_futuras_df = pd.DataFrame(previsoes_futuras_desnormalizadas, columns=['Previsão'])

'''
# Plot das previsões
plt.figure(figsize=(14, 6))
plt.plot(normalizador.inverse_transform(df_normalizado), color='blue', alpha=0.3, label='Dados Reais')
plt.plot(range(window_size, window_size + len(previsao_treinamento_desnormalizada)),
         previsao_treinamento_desnormalizada, color='orange', label='Previsão (Treino)')
plt.plot(range(window_size + tam_treinamento, window_size + tam_treinamento + len(previsao_teste_desnormalizada)),
         previsao_teste_desnormalizada, color='red', label='Previsão (Teste)')
plt.axvline(x=window_size + tam_treinamento, color='black', linestyle='--', label='Divisão Treino/Teste')
plt.title('Previsão de Casos de Dengue em Fortaleza')
plt.xlabel('Semanas')
plt.ylabel('Casos Estimados')
plt.legend()
plt.grid(True)
